...
function validateConditional<T>(
...
async function validateField<T>(
    fieldName: string,
    origFieldSchema: FieldSchema<T>,
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
    value: any,
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
    data: Record<string, any>
): Promise<{ error?: string; errorCode?: string; errorType?: string }> {
    ...
    switch (fieldSchema.type) {
        ...
        case 'color': {
            ...
            if (fieldSchema.minLength && validatedValue.length < fieldSchema.minLength) {
                return {
                    error:
                        fieldSchema.minLengthErrorMessage ||
            ...
            if (fieldSchema.maxLength && validatedValue.length > fieldSchema.maxLength) {
                return {
                    error:
                        fieldSchema.maxLengthErrorMessage ||
            ...
            if (fieldSchema.format) {
                ...
                if (regex && !regex.test(validatedValue)) {
                    return {
                        error:
                            fieldSchema.formatErrorMessage ||
        ...
        case 'float': {
            ...
            if (fieldSchema.exclusiveMin !== undefined && validatedValue <= fieldSchema.exclusiveMin) {
                return {
                    error:
                        fieldSchema.exclusiveMinErrorMessage ||
            ...
            if (fieldSchema.exclusiveMax !== undefined && validatedValue >= fieldSchema.exclusiveMax) {
                return {
                    error:
                        fieldSchema.exclusiveMaxErrorMessage ||
            ...
            if (fieldSchema.precision !== undefined) {
                ...
                if (Math.round(validatedValue * factor) / factor !== validatedValue) {
                    return {
                        error:
                            fieldSchema.precisionErrorMessage ||
        ...
        case 'array': {
            ...
            if (fieldSchema.minItems && validatedValue.length < fieldSchema.minItems) {
                return {
                    error:
                        fieldSchema.minItemsErrorMessage ||
            ...
            if (fieldSchema.maxItems && validatedValue.length > fieldSchema.maxItems) {
                return {
                    error:
                        fieldSchema.maxItemsErrorMessage ||
            ...
            if (fieldSchema.uniqueItems && new Set(validatedValue).size !== validatedValue.length) {
                return {
                    error:
                        fieldSchema.uniqueItemsErrorMessage ||
            ...
            if (fieldSchema.contains !== undefined && !validatedValue.includes(fieldSchema.contains)) {
                return {
                    error:
                        fieldSchema.containsErrorMessage ||
            ...
            if (fieldSchema.elementType) {
                for (let i = 0; i < validatedValue.length; i++) {
                    ...
                    if (errResult.error) return errResult;
                }
            }
            break;
        }
        ...
        case 'literal': {
            if (validatedValue !== fieldSchema.literal) {
                return {
                    error:
                        fieldSchema.literalErrorMessage ||
        ...
        case 'tuple': {
            ...
            if (fieldSchema.tupleSchemas && validatedValue.length !== fieldSchema.tupleSchemas.length) {
                return {
                    error:
                        fieldSchema.tupleLengthErrorMessage ||
...
export async function validateData<T extends Record<string, any>>(
    ...
    const errors: Partial<Record<keyof T, { error?: string; errorCode?: string; errorType?: string }>> = {};
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
    ...
    return {
        success,
        errors: errors as { [K in keyof T]?: { error: string; errorCode: string; errorType: string } },
        data: success ? (validatedData as T) : undefined,
    };
}

/* parse() will throw an error if validation fails */
// biome-ignore lint/suspicious/noExplicitAny: <explanation>
export async function parse<T extends Record<string, any>>(
    schema: FormSchema<T>,
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
    data: Record<string, any>
): Promise<T> {
...
// biome-ignore lint/suspicious/noExplicitAny: <explanation>
export async function safeParse<T extends Record<string, any>>(
    schema: FormSchema<T>,
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
    data: Record<string, any>
): Promise<ValidationResult<T>> {
