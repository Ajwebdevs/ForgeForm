...
import { FormSchema, FieldSchema, FieldType } from "../types/index.js";
...
export function createSchema<T extends Record<string, any>>(input: any): FormSchema<T> {
    ...
    if (!isValidSchema) {
        const errors =
            validateSchema.errors?.map(
                (err) => `${err.keyword} error at ${err.instancePath}: ${err.message}`
    ...
    try {
        ...
        return formSchema as FormSchema<T>;
        // biome-ignore lint/suspicious/noExplicitAny: <explanation>
    } catch (error: any) {
...
// biome-ignore lint/suspicious/noExplicitAny: <explanation>
function parseJsonSchema(jsonSchema: any): FormSchema<Record<string, any>> | FieldSchema<any> {
    if (jsonSchema.properties) {
        // It's an object schema.
        // biome-ignore lint/suspicious/noExplicitAny: <explanation>
        const fields: FormSchema<Record<string, any>>['fields'] = {};
        for (const propertyName in jsonSchema.properties) {
            if (Object.prototype.hasOwnProperty.call(jsonSchema.properties, propertyName)) {
                ...
                // biome-ignore lint/suspicious/noExplicitAny: <explanation>
                fields[propertyName] = parseJsonSchema(propertySchema) as FieldSchema<any>;
            }
        }
        return { fields };
    }
    // It's a FieldSchema.
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
    const fieldSchema: FieldSchema<any> = {
    ...
    if (jsonSchema.oneOf || jsonSchema.anyOf) {
        ...
        // biome-ignore lint/suspicious/noExplicitAny: <explanation>
        fieldSchema.types = (jsonSchema.oneOf || jsonSchema.anyOf).map(parseJsonSchema) as FieldSchema<any>[];
    }

    if (jsonSchema.type === "array" && jsonSchema.items) {
        // biome-ignore lint/suspicious/noExplicitAny: <explanation>
        fieldSchema.elementType = parseJsonSchema(jsonSchema.items) as FieldSchema<any>;
    }

    if (jsonSchema.type === "object" && jsonSchema.properties) {
        fieldSchema.type = "object";
        // biome-ignore lint/suspicious/noExplicitAny: <explanation>
        fieldSchema.schema = parseJsonSchema(jsonSchema) as FormSchema<any>;
    }

    return fieldSchema;
}

function mapJsonSchemaTypeToFieldType(jsonSchemaType: string | string[]): FieldType {
...
// biome-ignore lint/suspicious/noExplicitAny: <explanation>
function mapJsonSchemaFormatToFieldFormat(jsonSchemaFormat: string | undefined): FieldSchema<any>['format'] | undefined {
    switch (jsonSchemaFormat) {
        ...
        case "password":
            return "password";
        default:
            return jsonSchemaFormat as string | undefined;
    }
}
